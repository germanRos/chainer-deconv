...
CLASSES				= 3
NEWSIZE				= (360, 480)
MODEL_NAME			= '/home/gros/Projects/chainer-1.6.1/semseg/tmp_FCN_nonorm_5classes_e5/FCN_nonorm_5classes_e5_190.model'
...
def inference():
	#ipdb.set_trace()
	if(not os.path.exists(OUTPUT + '/COMB')):
		os.makedirs(OUTPUT + '/COMB')
	if(not os.path.exists(OUTPUT + '/RGB')):
		os.makedirs(OUTPUT + '/RGB')
	if(not os.path.exists(OUTPUT + '/GT')):
...
#		batchRGBn = 127 * batchRGBn / max(min_, max_)
		...
		for j in range(k):
			...
			p2 = (0.7*p + 0.3*rgb).astype('uint8')
			misc.imsave(OUTPUT + '/COMB/' + '{:06d}'.format(ID) + '.png', p2)
			misc.imsave(OUTPUT + '/RGB/' + '{:06d}'.format(ID) + '.png', rgb)
			misc.imsave(OUTPUT + '/GT/' + '{:06d}'.format(ID) + '.png', p)
			axarr3.imshow(p2)
			plt.draw()
			plt.pause(0.01)
